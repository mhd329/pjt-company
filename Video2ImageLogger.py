...
# 로거 객체 생성 및 설정
log_path = "./logs"
v2i_logger = logging.getLogger(__name__) # 로거가 사용되는 네임스페이스를 따름
v2i_logger.setLevel(logging.INFO) # 기본 INFO level 까지 로깅 (필요시 개별 파일마다 별도로 로그 레벨 설정가능)

# 일반 로깅
formatter = logging.Formatter("%(asctime)s %(levelname)s : %(processName)s %(threadName)s %(funcName)s() -> %(message)s")
file_handler = logging.FileHandler(f"{log_path}/{now}_events.log")
...
critical_file_handler = logging.FileHandler(f"{log_path}/{now}_critical_events.log")
